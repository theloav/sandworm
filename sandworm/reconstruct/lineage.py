...
class LineageIndex:
    ...
    def neighbours(self, sig: Signature, *, threshold: float = 0.5, top: int = 10) -> list[Neighbour]:
        out = [
            Neighbour(other, round(jaccard(sig.minhash, other.minhash), 3))
            for sha, other in self.sigs.items() if sha != sig.sha256
        ]
        out = [n for n in out if n.similarity >= threshold]
